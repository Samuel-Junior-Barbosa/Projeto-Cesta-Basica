...
const HistoryBasicFoodBasket = () => {
    ...
    const selectThisBasket = async () => {
        ...
        */
        let product = []
        let productResponse = await GetHistoryBasketModel(idHistoryModel)
        if( productResponse.status === 0) {
            product = productResponse.content.itensDaCesta
        }

        console.log('Produtos: ', product)
        navigate('/input-and-output-baskets', { state: { typeAction: 'Saida', dataOfBasket: '', dataProdutosRecived: product}});
    };
