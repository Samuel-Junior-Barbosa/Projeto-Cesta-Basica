...
import { useInsertItemOnBasketModel } from '../../../Components/hooks/ManageBasicFoodBaskets/InsertItemOnBasketModel';
...
const ListOfBasicBasketItems = () => {
    ...
    const { handleInsertItemOnBasketModel, InsertItemOnBasketModelLoading, InsertItemOnBasketModelMessage} = useInsertItemOnBasketModel()
    ...
    const handleRemoveItemOnTable = () => {
        ...
        //console.log(" removeItens: ", Object.values(removeItens[0]))
        for( let i = 0; i >= 0; i++ ) {
            //console.log(" removeItens: ", Object.values(removeItens[i]))
            removeItemOfBasketModel(basketId, Object.values(removeItens[i])[0], )
        }
    ...
    const handleSaveAlteration = () => {
        ...
        for( let i = 0; i < basketItemList.length; i ++ ) {
            ...
            removeItemOfBasketModel( basketId, basketItemList[i].id)
        }

        for( let i = 0; i < basketItemList.length; i ++ ) {
            let basketData = {
                productId : basketItemList[i]["id"],
                idBasket : basketId,
                productName : basketItemList[i][],
                productQuantity : Number(basketItemList[i]["Quantidade de itens"])
            }

            //console.log(" basketDATA index: ", basketData)
            handleInsertItemOnBasketModel( basketData )
        }
    ...
    useLayoutEffect(() => {
        ...
        if( addItemContent && Object.keys(addItemContent).length > 0) {
            ...
            let basketData = {
                id : addItemContent[0],
                ["idBasket"] : basketId,
                ["produto"] : addItemContent[1],
                ["quantidade"] : Number(addItemContent[3])
            }

            
            setBasketItemList([..., basketItemList])
    ...
    return (
        <div className={styles.listBasicBasketItensDiv}>
            { InsertItemOnBasketModelMessage && (
                <MessageAlert
                    text={InsertItemOnBasketModelMessage}
                >
                </MessageAlert>
            )}
